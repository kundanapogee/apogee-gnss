...
if (isset($_POST['sequence_no'])) {
	...
	// $query = $conn->prepare("SELECT * from product_sub_type where id = :id and sequence_no = :sequence_no and product_sub_type_id = :product_sub_type_id");
	...
	$query = $conn->prepare("SELECT * from product_sub_type where product_type_id = :product_type_id and sequence_no = :sequence_no");
	$query->bindParam(':product_type_id',$product_type_id);
	$query->bindParam(':sequence_no',$sequence_no);
	// $query->bindParam(':product_sub_type_id',$product_sub_type_id);
	$query->execute();
